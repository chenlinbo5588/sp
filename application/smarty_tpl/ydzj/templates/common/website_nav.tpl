			<ul id="homeNav">
				{foreach from=$siteNavs item=item key=key}
       			<li class="level0 {if $currentModule == 'home'}current{/if}"><a class="link0" href="{str_replace($idReplacement,$item['id'],$item['url_cn'])}" {if $item['jump_type'] == 1}target="_blank"{/if}>{$item['name_cn']|escape}</a>
       				{if $item['children']}
       				<ul class="sublist">
