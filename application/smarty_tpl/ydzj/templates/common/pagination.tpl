<div class="pagination" data-formid="{$page['form_id']}">
    {if ($page['pageLastNum'] > 1)}
        ...
        {section name=a loop=($page['pageAe'] - $page['pageAb'] + 1)}
            <a {if ($page['pageNow'] == ($smarty.section.a.index + $page['pageAb']))}class="active"{/if} {if $page['call_js']}href="javascript:void(0)" onclick="{$page['call_js']}({$smarty.section.a.index + $page['pageAb']},'{$page['form_id']}');return false;" {else}href="{$page['base_link']}&page={$smarty.section.a.index + $page['pageAb']}#{$page['anchor']}"{/if}>{$smarty.section.a.index + $page['pageAb']}</a>
        {/section}
