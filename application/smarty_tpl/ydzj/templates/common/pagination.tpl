<div class="pagination" data-formid="{$page['form_id']}">
    {if ($page['pageLastNum'] > 1)}
        ...
        {else}
            <a {if $page['call_js']}href="javascript:void(0)" onclick="{$page['call_js']}(1);return false;"{else}href="{$page['base_link']}&page=1"{/if}>第一页</a>
        {/if}
        ...
        {/if}
        <strong><label>找到{$page['pageSum']}{$page['pageUnit']}{$page['pageTit']}</label>{if $page['shortStyle'] == false}<input type="text" style="width:40px" name="jumpPage" value="{$page['pageNow']}" />&nbsp;<input type="button" name="jumpBtn" value="跳转" class="btn orangeBtn jumpBtn" />&nbsp;
        {/if}
