...
<div id="create_team" class="handle_area">
    ...
    <div class="row">
        ...
        <label><input type="radio" name="leader" value="1" {set_radio('leader',1)}/>我是队长</label>
    ...
    <div class="row">队伍合影照片<em class="tip">JPG</em>格式,大小不超过<em class="tip">2MB</em>，分辨率大于<em class="tip">100X100</em></div>
    {form_error('joined_type')}
    <div class="row">
        <label class="side_lb">入队设置：</label>
        <label><input type="radio" name="joined_type" value="0" {set_radio('joined_type',0)}/>无需验证</label>
        <label><input type="radio" name="joined_type" value="1" {set_radio('joined_type',1)}/>需要验证</label>
        <label><input type="radio" name="joined_type" value="2" {set_radio('joined_type',2)}/>通过邀请加入</label>
    </div>
    <div class="row">
