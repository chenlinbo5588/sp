    ...
    <form action="{site_url($uri_string)}" method="get" id="formSearch">
        ...
        <ul class="color_list clearfix">
            ...
            <li class="list_item" id="row{$item['id']}">
                <label><input type="checkbox" name="id[]" group="id" value="{$item['id']}"/>{$item['color_name']|escape}&nbsp;&nbsp;&nbsp;<a class="delete" data-title="删除"  href="javascript:void(0)" data-url="{site_url('my_color/delete')}" data-id="{$item['id']}">X</a></label>
            </li>
            {foreachelse}
