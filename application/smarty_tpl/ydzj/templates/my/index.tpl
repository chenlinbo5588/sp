...
<div id="my" class="bordered">
    ...
    <div class="pd5">
	    ...
	    <div class="row"><span class="icon ico_email">QQ:</span><span>{$profile['basic']['qq']}</span>{if $profile['basic']['qq'] == ''}<a class="side_link" href="{site_url('my/set_email')}">设置QQ</a>{/if}</div>
        <div class="row"><span class="icon ico_email">微信号:</span><span>{$profile['basic']['weixin']}</span>{if $profile['basic']['weixin'] == ''}<a class="side_link" href="{site_url('my/set_email')}">设置微信号</a>{/if}</div>
