...
{config_load file="goods.conf"}
  {if $info['goods_id']}
  {form_open_multipart(site_url($uri_string|cat:'?goods_id='|cat:$info['goods_id']),'id="goods_form"')}
  {else}
  {form_open_multipart(site_url($uri_string),'id="goods_form"')}
  {/if}
      ...
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform">
	          ...
	          <option {if $info['gc_id'] == $item['gc_id']}selected{/if} value="{$item['gc_id']}">{str_repeat('......',$item['level'])}{$item['level']+1} {$item['name']}</option>
	          {/foreach}
	        </select>
          </td>
