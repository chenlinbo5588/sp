{include file="common/main_header.tpl"}
  ...
  {if $info['gc_id']}
  {form_open(admin_site_url('brand/edit'),'id="brand_form"')}
  {else}
  {form_open(admin_site_url('brand/add'),'id="brand_form"')}
  {/if}
      ...
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><span class="type-file-show"><img class="show_image" src="{resource_url('img/preview.png')}">
            ...
            <input type='text' name='textfield' value="{if $info['brand_pic']}{resource_url($info['brand_pic'])}{/if}" id='brand_pic' class='type-file-text' />
            <input type='button' name='button' id='button1' value='' class='type-file-button' />
            <input name="brand_logo" type="file" class="type-file-file" id="brand_logo" size="30" hidefocus="true" nc_type="change_site_logo">
            </span></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform onoff"><label for="brand_recommend1" {if $info['brand_recommend']}class="cb-enable selected"{else}class="cb-enable"{/if}><span>是</span></label>
            <label for="brand_recommend0" {if $info['brand_recommend']}class="cb-disable selected"{else}class="cb-disable"{/if}><span>否</span></label>
