  ...
  <input type="hidden" name="id" value="{$info['id']}"/>
      ...
      <tbody>
        ...
        *}
        ...
        <tr class="noborder">
          <td colspan="2"><label>{#article_state#}: {$article_state[$info['article_state']]}</label></td>
        </tr>
        <tr class="noborder">
          <td colspan="2">{$info['verify_reason']|escape}</td>
        </tr>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><textarea name="reason" style="width:300px;height:80px;"></textarea></td>
          <td class="vatop tips">{form_error('reason')}</td>
        </tr>
        {/if}
      </tbody>
    </table>
   ...
   <script type="text/javascript">
	$(function(){
		$.loadingbar({ text: "正在提交..." , urls: [new RegExp("{site_url($uri_string)}")] , container : "#article_form" });
		bindAjaxSubmit("#article_form");
	});
