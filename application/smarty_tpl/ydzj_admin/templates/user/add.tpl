{include file="common/main_header_navs.tpl"}
  {if $info['uid']}
  {form_open(admin_site_url('user/edit'),'id="add_form"')}
  {else}
  {form_open(admin_site_url('user/add'),'id="add_form"')}
  {/if}
    ...
    <table class="table tb-type2 nobdb">
      <tbody>
        ...
        <tr class="noborder">
          ...
          <td class="vatop tips">请选择一个权限组，如果还未设置，{form_error('gid')} <a href="{admin_site_url('role/add')}">点击马上设置</a></td>
        </tr>
      </tbody>
      <tfoot>
        <tr class="tfoot">
