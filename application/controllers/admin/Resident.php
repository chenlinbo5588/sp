...
class Resident extends Ydzj_Admin_Controller {
	...
	private function _getRules(){
		...
		$this->form_validation->set_rules('brand_recommend','是否推荐','required|in_list[0,1]');
	...
	public function add(){
		...
		if($this->isPostRequest()){
			...
			for($i = 0; $i < 1; $i++){
				...
				if(($newid =$this->Resident_Model->_add($info)) < 0){
					$feedback = getErrorTip('保存失败');
					break;
				}
				
				$feedback = getSuccessTip('保存成功');
				$info = $this->Resident_Model->getFirstByKey($newid);
	...
	public function edit(){
		...
		$info = $this->Resident_Model->getFirstByKey($id,'brand_id');
		...
		if($this->isPostRequest()){
			...
			for($i = 0; $i < 1; $i++){
				...
				if($this->Resident_Model->update($info,array('brand_id' => $id)) < 0){
					$feedback = getErrorTip('保存失败');
					break;
				}
