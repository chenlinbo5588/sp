...
class Proxy {
    ...
    public function verifyConfiguration()
    {
        ...
        if ($this->canProcessRequest()) {
			...
			//file_put_contents("debug.txt",print_r($this->resource,true));
        ...
        if ($this->proxyConfig['mustmatch'] == false || $this->proxyConfig['mustmatch'] === "false") {
            ...
            foreach ($this->serverUrls as $key => $value) {
                ...
                if ($s['matchall'] == true || $s['matchall'] === "true") {
                    ...
                    $urlStartsWith = $this->startsWith($s['url'], $this->proxyUrl);
    ...
    public function runProxy()
    {
        ...
        $this->proxyLog->log("proxyMethod:" . $this->proxyMethod);
    ...
    public function proxyPost($url = '', $params = '')
    {
        ...
        file_put_contents("debug.txt",print_r($url,true));
    	file_put_contents("debug.txt",print_r($params,true),FILE_APPEND);
        ...
        try {
            ...
            file_put_contents("debug.txt",print_r($this->response,true),FILE_APPEND);
    ...
    {
        try {
//file_put_contents("debug.txt",print_r($this->proxyData,true));
//file_put_contents("debug.txt",print_r($_FILES,true),FILE_APPEND);
            ...
            $this->response = curl_exec($this->ch);
//file_put_contents("debug.txt",print_r($query_array,true));
//file_put_contents("debug.txt",print_r($_FILES,true),FILE_APPEND);
    ...
    function startsWith($requested, $needed)
    {
        ...
        return stripos($requested, $needed) !== false;
...
class ProxyConfig {
    ...
    public function useXML($configFile = '')
    {

        $xmlParser = new XmlParser($configFile);
...
$proxyConfig->useXML(ROOT_PATH.'/application/config/proxy.config');

//$proxyConfig->useJSON();
//file_put_contents("proxyConfig.txt",print_r($proxyConfig,true));
